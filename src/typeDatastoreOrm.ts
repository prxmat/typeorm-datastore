    ...
    public getInsertData<T extends BaseEntity>(entity: T): IGetInsertData {
        ...
        if (this.isEmptyKey(key)) {
            if (isGenerateId) {
                ...
                throw new TypeDatastoreOrmError(`_id must not be 0, empty string or undefined.`);
            }
        } else {
    ...
    public validateKey(keys: DatastoreEntity.entity.Key | DatastoreEntity.entity.Key[],
        ...
        for (const key of (Array.isArray(keys) ? keys : [keys])) {
            if (key.namespace !== namespace) {
                throw new TypeDatastoreOrmError(`Namespace not match. Entity namespace is "${key.namespace}". While the expected namespace is "${namespace}".`);
            }

            if (key.kind !== kind) {
                throw new TypeDatastoreOrmError(`Kind not match. Entity kind is "${key.kind}". While the expected kind is "${kind}".`);
            }
...
export const typeDatastoreOrm = new TypeDatastoreOrm();